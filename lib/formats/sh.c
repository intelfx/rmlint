...
static const char SH_SCRIPT_TEMPLATE[] = {
#if HAVE_C23_EMBED
#embed "sh.sh"
#else
#include "sh.sh.c.inc"
...
static const char SH_SCRIPT_SEPARATOR[] =
...
static void rm_fmt_sh_template_init(RmFmtHandlerShScript *self) {
    const char *sep = strstr(SH_SCRIPT_TEMPLATE, SH_SCRIPT_SEPARATOR);
    g_assert_nonnull(sep);

    self->tpl_head = g_strndup(SH_SCRIPT_TEMPLATE, sep - SH_SCRIPT_TEMPLATE);
    self->tpl_foot = g_strdup(sep + strlen(SH_SCRIPT_SEPARATOR));
}

static void rm_fmt_sh_template_fini(RmFmtHandlerShScript *self) {
    g_free(self->tpl_foot);
    g_free(self->tpl_head);
}
...
static void rm_fmt_head(RmSession *session, RmFmtHandler *parent, FILE *out) {
    ...
    /* Split the template into header and footer */
    rm_fmt_sh_template_init(self);
...
static void rm_fmt_foot(_UNUSED RmSession *session, RmFmtHandler *parent, FILE *out) {
    ...
    /* Fill in all placeholders in the script template footer */
    fprintf(
        out, self->tpl_foot,
        escaped_path
    );
    ...
    const char *progress_marker = strstr(self->tpl_head, progress_marker_text);
    if(progress_marker != NULL) {
        gsize offset = (progress_marker - self->tpl_head) +
